...
async def start(client:Client, message):
    ...
    if len(m.command) == 2 and m.command[1].startswith('notcopy'):
        ...
        if await db.user_verified(user_id):
            ...
            key = "second_time_verified" if 
        await db.is_user_verified(user_id) else "first_time_verified"
...
current_time = dt.now(tz=ist_timezone)  # Yeh line properly aligned hai
        ...
        msg = script.VERIFY_COMPLETE_TEXT  # Default message
        ...
        await client.send_message(settings['log'], script.VERIFIED_LOG_TEXT.format(
            m.from_user.mention, user_id, dt.now(pytz.timezone('Asia/Kolkata')).strftime('%d %B %Y'), num))
...
# Command to buy premium membership
...
async def buy_premium(client, message):
    ...
    order_id = payment['id']
    pending_payments[user_id] = order_id  # Store order ID for verification

    payment_link = f"https://rzp.io/l/{order_id}"  # Replace with actual Razorpay link format

    await message.reply(
        f"🔹 **Premium Membership** 🔹\n\n💰 Price: ₹{amount}\n🔗 Pay here: {payment_link}\n\n"
        "Once payment is done, send /verify_payment"
    )

# Command to verify payment
@Client.on_message(filters.command("verify_payment") & filters.private)
async def verify_payment(client, message):
    user_id = message.from_user.id

    if user_id not in pending_payments:
        await message.reply("❌ No pending payment found. Please use /buy_premium first.")
        return

    order_id = pending_payments[user_id]

    try:
        # Fetch payment details from Razorpay
        payment = razorpay_client.order.fetch(order_id)
        if payment["status"] == "paid":
            # Mark user as premium in the database
            await db.set_premium(user_id, True)
            del pending_payments[user_id]  # Remove from pending list
            await message.reply("✅ Payment verified! You are now a premium user.")
        else:
            await message.reply("❌ Payment not completed. Please try again.")
    except Exception as e:
